...
try:
    while True:
        print ('---------------------------------------')
        print("Please enter your key to use in root stream")
        keyToUse = raw_input('Input: ')
        temp = TemperaturAuswertung()
        print "Temperatur:", temp, "°C"
        api.publish("root", keyToUse, converter.hexlify(str(temp)))
        time.sleep(sleeptime)
 
except KeyboardInterrupt:
	print("Interrupt happened, closing GPIO")
	GPIO.cleanup()
